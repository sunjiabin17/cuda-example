...
__global__ void sgemm6(int M, int N, int K, float alpha, float *A, int lda, float *B, int ldb,
...
void test_sgemm6(int M, int N, int K, float *alpha, float *dA, int lda, float *dB, int ldb, 
                float *beta, float *dC, int ldc) {
    dim3 block(16, 16);
    dim3 grid((M + Ms128 - 1) / Ms128, (N + Ns128 - 1) / Ns128);
    
    sgemm6<<<grid, block>>>(M, N, K, *alpha, dA, lda, dB, ldb, *beta, dC, ldc);
