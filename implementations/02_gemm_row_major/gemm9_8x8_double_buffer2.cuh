...
__global__ void sgemm9(int M, int N, int K, float alpha, float *A, int lda, float *B, int ldb,
    ...
    // __shared__ float a_shared[0][Ms128][Ks8];
    __shared__ float a_shared[2][Ks8][Ms128];  // here, transpose a_shared[0] after loaded from global to shared
    ...
    // 存入共享内存
    a_shared[0][col_a][row_a] = pref_Av.x;                  // transpose A
    a_shared[0][col_a+1][row_a] = pref_Av.y;
    a_shared[0][col_a+2][row_a] = pref_Av.z;
    a_shared[0][col_a+3][row_a] = pref_Av.w;
    *((float4 *)(&b_shared[0][row_b][col_b])) = pref_Bv;   // 方法2
    __syncthreads();

    // 2. 加载第一个A的8个元素和B的8个元素到寄存器Av[0] Bv[0], 准备做向量外积
    Av1[0] = *((float4 *)(&a_shared[0][0][row_c]));
    Av2[0] = *((float4 *)(&a_shared[0][0][row_c+4]));
    Bv1[0] = *((float4 *)(&b_shared[0][0][col_c]));
    Bv2[0] = *((float4 *)(&b_shared[0][0][col_c+4]));

    for (int k_idx = 0; k_idx < K/Ks8; k_idx++) {
        // 已经加载了第一个Ks块, 提前做偏移
        pA = pA + Ks8;
        pB = pB + Ks8 * ldb;
        // 3. 加载下一个Ks大小块
        ...
        for (int kk = 0; kk < Ks8; ++kk) {
            ...
            Av1[(kk+1)&1] = *((float4 *)(&a_shared[(k_idx)&1][(kk+1)%Ks8][row_c])); // A中的4个连续的数分别对应C的每一行
            Av2[(kk+1)&1] = *((float4 *)(&a_shared[(k_idx)&1][(kk+1)%Ks8][row_c+4]));
            Bv1[(kk+1)&1] = *((float4 *)(&b_shared[(k_idx)&1][(kk+1)%Ks8][col_c]));
            Bv2[(kk+1)&1] = *((float4 *)(&b_shared[(k_idx)&1][(kk+1)%Ks8][col_c+4]));
        ...
        a_shared[(k_idx+1)&1][col_a][row_a] = pref_Av.x;
        a_shared[(k_idx+1)&1][col_a+1][row_a] = pref_Av.y;
        a_shared[(k_idx+1)&1][col_a+2][row_a] = pref_Av.z;
        a_shared[(k_idx+1)&1][col_a+3][row_a] = pref_Av.w;
        *((float4 *)(&b_shared[(k_idx+1)&1][row_b][col_b])) = pref_Bv;
        __syncthreads();
        // 6. 预加载下一次大循环开始的A的8个元素和B的8个元素到寄存器Av[0] Bv[0]
        Av1[0] = *((float4 *)(&a_shared[(k_idx+1)&1][0][row_c]));
        Av2[0] = *((float4 *)(&a_shared[(k_idx+1)&1][0][row_c+4]));
        Bv1[0] = *((float4 *)(&b_shared[(k_idx+1)&1][0][col_c]));
        Bv2[0] = *((float4 *)(&b_shared[(k_idx+1)&1][0][col_c+4]));
    }
